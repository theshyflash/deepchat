...
import { useSelectStore } from '@/stores/Lang'
...
const useLang = useSelectStore()
onMounted(() => {
  console.log("数据已经存储:" + useLang.LangFirst);
  console.log("数据已经存储:" +useLang.LangSecond);
  initSettings()
  ...
  ipcRenderer?.on('variable-changed',  (_, msg) => {
    ...
    // inputText.value = msg
    inputText.value = msg
    configPresenter.setLangFirst(useLang.LangFirst)
    configPresenter.setLangSecond(useLang.LangSecond)
    configPresenter.getLangFirst().then((msg) =>{
      console.log("刚开的内容:"+msg)
    })
    configPresenter.getLangSecond().then((msg)=>{
      console.log('最后要翻译的内容:',msg)
    })
